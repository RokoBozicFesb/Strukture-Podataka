...
int Hashracun(char* V, int velTab);
void addtohash(char*);
void printhash();


hashP hashTable[11] = { NULL };
...
void addtohash(char* key) {
	int index = Hashracun(key, 11);

	hashP newhash = (hashP)malloc(sizeof(hash));
	strcpy(newhash->key, key);
	newhash->countryptr = NULL;
